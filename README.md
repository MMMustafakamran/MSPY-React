...
**Tracked without a demo.** Four pages carry a route, a nav entry and a snapshot so drift is watched, but nothing is implemented behind them and the recorder does not touch them: `/ms-agent-python/webmcp`, `/ms-agent-python/human-in-the-loop/governed-actions`, and both `/generative-ui/a2ui/*` pages. The reason is on each route’s page and in §7. The A2UI parents and the rest of `/ms-agent-python/intelligence/` stay in `doc-snapshot/manifest.json`’s `knownUnmapped` list.
