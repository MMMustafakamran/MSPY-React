...
<Steps>
  <Step>
    ...
    Complete the [Intelligence quickstart](/ms-agent-python/intelligence/quickstart). That page signs you in with the CLI and selects the project. Then send a message and make sure that it appears in [Rich Threads](/ms-agent-python/threads). Open your project in [CopilotKit Intelligence](https://dashboard.operations.copilotkit.ai/) and make sure that Learning is available.
...
<Steps>
  ...
  <Step>
    ...
    For manual setup, follow the [framework adapter examples](/ms-agent-python/intelligence/learned-skills#native-setup). Pick the adapter for the agent you already run. Configure that adapter in the agent server environment with the same project key and container ID used above.
    ...
    Keep the key server-side. Leave `CPK_INTELLIGENCE_SKILLS_REVISION` unset to follow the latest published Skills. See [container configuration](/ms-agent-python/intelligence/learned-skills#configure-one-container) for self-hosted endpoints and exact revision pins.
  </Step>
...
| Delivery status is unavailable              | Select **Retry delivery status**. For self-hosted deployments, verify the delivery endpoint and migration are installed before relying on an adapter. See [deployment requirements](/ms-agent-python/intelligence/learned-skills#deployment-requirements). |

## Related guides

- [CopilotKit Intelligence overview](/ms-agent-python/intelligence/overview)
- [Connect CopilotKit Intelligence](/ms-agent-python/intelligence/quickstart)
- [Rich Threads](/ms-agent-python/threads)
- [Threads & Persistence Architecture](/ms-agent-python/intelligence/threads-explained)
- [CopilotKit CLI](/ms-agent-python/cli)
