...
and an [AG-UI middleware](/ms-agent-python/agentic-protocols/ag-ui-middleware) that applies it.
...
[`selfManagedAgents` or the local development option](/ms-agent-python/backend/self-managed-agents).
...
- [AG-UI Middleware](/ms-agent-python/agentic-protocols/ag-ui-middleware): the middleware
  lifecycle and the `Middleware` base class.
- [Self-managed agents](/ms-agent-python/backend/self-managed-agents): passing your own agent
  instances to the frontend provider.
- [AgentRunner and persistence](/ms-agent-python/backend/agent-runner): where the runtime keeps
  thread history.
